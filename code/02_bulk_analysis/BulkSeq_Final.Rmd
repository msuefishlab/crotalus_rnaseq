...
## New Analysis: Potassium Channel Heatmap
```{r}
...
genes_we_have<-which(rownames(dds) %in% kv_channels)

kv_names_we_have<-kv_table[rownames(dds[genes_we_have,]),]

kv_dds<-dds[genes_we_have,]

indices <- match(rownames(kv_dds), rownames(kv_table))
rownames(kv_dds) <- kv_table$Names[indices]

tcounts <- t(log2((counts(kv_dds, normalized=TRUE, replaced=FALSE)+.5))) %>%
  merge(colData(dds), ., by="row.names") %>%
  gather(gene, expression, (ncol(.)-length(kv_dds)+1):ncol(.))


tcounts$expression<-as.numeric(tcounts$expression)
tcounts$Row.names <- as.character(tcounts$Row.names)
tcounts$gene <- as.character(tcounts$gene)

ggplot(tcounts, aes(x = Row.names, y = gene)) +
  geom_tile(aes(fill = expression), color = "white") +
  scale_fill_gradient2(low = "white", mid = "yellow", high = "red", midpoint =4) +
  theme_minimal() +
  labs(title = "Potassium Channel Heatmap - Bulk Sequencing", x = "Row Names", y = "Gene") +
